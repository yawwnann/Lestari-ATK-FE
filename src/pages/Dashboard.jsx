...
import AtkImage from "../assets/atk.png"; // Ganti gambar ini untuk ATK
...
function Dashboard() {
  const [atkList, setAtkList] = useState([]); // <--- Diubah: pupukList -> atkList
  ...
  const fetchAtk = async (page = 1) => {
    // <--- Diubah: fetchPupuk -> fetchAtk
    setLoading(true);
    try {
      const response = await apiClient.get(`/atk?page=${page}`); // <--- Diubah: /pupuk -> /atk
      setAtkList(response.data.data); // <--- Diubah: setPupukList -> setAtkList
  ...
  const viewDetail = (slug) => {
    // <--- Diubah: viewDetail
    navigate(`/atk/${slug}`); // <--- Diubah: /pupuk -> /atk
  };
  ...
  // Animasi untuk Atk Cards (Dipicu Scroll SETELAH loading selesai)
  useEffect(() => {
    if (!loading && atkList.length > 0) {
      // <--- Diubah: pupukList -> atkList
      gsap.fromTo(
        ".atk-card", // <--- Diubah: .pupuk-card -> .atk-card
        { opacity: 0, y: 50, scale: 0.95 },
        {
          ...
          overwrite: "auto",
          scrollTrigger: {
            trigger: ".atk-grid", // <--- Diubah: .pupuk-grid -> .atk-grid
            start: "top 80%",
            toggleActions: "play none none none",
          },
        }
    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
        ...
        <div className="container mx-auto px-6 py-24 md:py-32 lg:py-40 relative z-10">
          <div className="max-w-xl lg:max-w-2xl text-center md:text-left relative z-10">
            <h1 className="hero-title text-4xl lg:text-5xl xl:text-6xl font-bold leading-tight mb-4">
              Selamat Datang di LESTARI ATK! {/* <--- Nama Perusahaan diubah */}
            </h1>
            <p className="hero-description text-lg lg:text-xl text-emerald-100 mb-8">
              {/* <--- Warna diubah */}
              Kelola pesanan ATK berkualitas Anda, lihat produk terbaru, dan
      ...
      <section className="container mx-auto px-6 py-16 text-center">
        <h2 className="tujuan-heading text-3xl font-bold text-gray-800 mb-12">
          ...
          {/* <--- Warna diubah */}
          DAN <span className="text-emerald-600">MENGHADIRKAN</span> ATK
        ...
        <div className="container mx-auto px-4 md:px-6">
          <div className="flex flex-col md:flex-row justify-between items-center mb-8">
            <h3 className="katalog-heading text-4xl md:text-5xl font-extrabold text-gray-900 mb-4 md:mb-0">
              Katalog ATK Terbaru {/* <--- Teks diubah */}
            </h3>
            {atkList.length > 8 && (
              // <--- Diubah: pupukList -> atkList
          ...
          {/* Tambahkan kelas 'atk-grid' di sini */}
          <div className="atk-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
            {loading
              ...
              : atkList.slice(0, 8).map((atk) => (
                  // <--- Diubah: pupukList -> atkList, pupuk -> atk
                  <div
                    key={atk.id}
                    className="atk-card group bg-white rounded-xl shadow-lg hover:shadow-2xl overflow-hidden transition-all duration-300 ease-in-out flex flex-col h-full transform hover:-translate-y-1" // <--- Diubah: pupuk-card -> atk-card
                  >
                    <div className="relative overflow-hidden">
                      <img
                        // Menggunakan nama_atk sebagai alt text
                        // Asumsi gambar_utama dari API sudah berupa URL lengkap dari Cloudinary
                        src={atk.gambar_utama} // <--- Menggunakan gambar_utama langsung (asumsi sudah URL lengkap)
                        alt={atk.nama_atk} // <--- Diubah: atk.nama_pupuk -> atk.nama_atk
                    ...
                    <div className="p-5 flex flex-col flex-grow">
                      <h3 className="text-xl font-bold text-gray-800 mb-2 line-clamp-2">
                        {atk.nama_atk}{" "}
                        {/* <--- Diubah: atk.nama_pupuk -> atk.nama_atk */}
                      </h3>
                      <p className="text-sm text-gray-600 mb-4 flex-grow line-clamp-3">
                        {atk.deskripsi || "Deskripsi ATK belum tersedia."}{" "}
                        {/* <--- Teks diubah */}
                      </p>
                      ...
                      <button
                        onClick={() => viewDetail(atk.slug)} // <--- Diubah: pupuk.slug -> atk.slug
      ...
      <section className="container mx-auto px-6 py-16 text-center">
        <h2 className="benefit-heading text-3xl font-bold text-gray-800 mb-12">
          Kenapa Memilih LESTARI ATK? {/* <--- Nama Perusahaan diubah */}
        </h2>
        <div className="benefit-grid grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          <div className="benefit-card bg-white p-6 rounded-lg shadow-md hover:shadow-xl transition duration-300 ease-in-out transform hover:-translate-y-1">
            ...
            <p className="text-gray-600 text-left">
              Kami hanya menyediakan ATK berkualitas dengan jaminan keaslian dan
              kualitas yang terjaga. {/* <--- Teks diubah */}
            </p>
          </div>
          <div className="benefit-card bg-white p-6 rounded-lg shadow-md hover:shadow-xl transition duration-300 ease-in-out transform hover:-translate-y-1">
            <div className="flex items-center justify-between mb-4">
            ...
            <p className="text-gray-600 text-left">
              Pengiriman cepat dengan berbagai opsi, menjamin ATK sampai tepat
          ...
          <div className="benefit-card bg-white p-6 rounded-lg shadow-md hover:shadow-xl transition duration-300 ease-in-out transform hover:-translate-y-1">
            ...
            <p className="text-gray-600 text-left">
              ...
              setiap pembelian ATK berkualitas. {/* <--- Teks diubah */}
            </p>
          </div>
        </div>
      </section>
